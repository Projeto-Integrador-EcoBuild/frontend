...
function CardCategoria({categoria}: CardCategoriaProps) {
  return (
    <div className='border flex flex-col rounded-1xl  justify-between border-gray-400 dark:text-white dark:border-green-dark'>
      <header className="py-2 px-6 bg-green-light dark:bg-green-dark h-36 flex justify-center items-center text-3xl text-center capitalize ">{categoria.nome}</header>
      <p className='p-6 text-justify text-2xl '>{categoria.descricao}</p>
      <div className="flex">
        <Link to={`/editarCategoria/${categoria.id}`} className=' bg-green-light text-center p-3 w-full text-2xl uppercase 
