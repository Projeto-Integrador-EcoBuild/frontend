    ...
    return (
      ...
      cp:w-[80%] lg:w-[70%] xl:w-[60%] dark:bg-gray-inputs dark:shadow-slate-900 dark:text-white" onSubmit={cadastrarNovoUsuario}>
                <div className='flex flex-row justify-around  w-[80%] items-center cp:w-full sm:w-full md:w-full lg:w-full' >
                    <h2 className="text-8xl font-light cp:text-5xl sm:text-7xl ">{id === undefined ? 'Cadastro' : 'Edição'}</h2>
                    {usuarioT.foto === '' ? (<img className=' w-32 h-32 cp:w-28 cp:h-28 rounded-full' src={UsuarioFotoVazio}></img>) : (<img className=' w-32 h-32 cp:w-28 cp:h-28 rounded-full object-cover' src={usuarioT.foto}></img>)}
                ...
                {id === undefined ? <p className='w-full text-center xl:text-xl lg:text-xl 2xl:text-xl'>
